...
%                 fprintf('already in state %s\n', name);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            %% START CODE BLOCK %%
            r_delta = (right_ticks - prev_right_ticks)*m_per_tick;
            l_delta = (left_ticks - prev_left_ticks)*m_per_tick;
            
            c_delta = (r_delta + l_delta) / 2;
            phi = (r_delta - l_delta)/L;
            
            x_dt = c_delta*cos(theta);
            y_dt = c_delta*sin(theta);
            theta_dt = phi;
